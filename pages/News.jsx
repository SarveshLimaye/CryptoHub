       ...
       <Title title={"News"} />
        ...
        {news.map((item,index) => {
            return (
                ...
                <Heading as='h3' ml={3} mt={6} mr={2} size='sm'>{item.name.length > 150 ? `${item.name.slice(0, 95)}`  : item.name } </Heading>
                <Center>
                <Box mt={4}>
                ...
                <Box>
                   ...
                   {item.provider.map((singleProvider) => {
                          return (
                            ...
                            {singleProvider.image === undefined ? null : <Image alt={'provider-image'} mt={-3} src={singleProvider.image.thumbnail.contentUrl} width={35} height={item.length > 150 ? 25 : 35 }  />}
                            </Box>
                            </Box> 
                          )
                   })}
